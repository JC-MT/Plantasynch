...
const sendEmail = async () => {
  ...
  <h2 style="padding: 5px;margin: 0px;">We see ${plants.length} ${
   ...
   <h4 style="padding: 5px;margin: 0px;">If you decide the plant is not ready to be watered, hit the skip button and Plantasynch will learn and give better suggestions for your specific plant</h4></div>`;
      let logo = `<div style="padding: 20px ;display:flex;align-self: center;place-items: center; place-content: center;">
            <img alt='logo' style='place-self: center; width: 40px; height: 40px;' src='https://cdn-icons-png.flaticon.com/512/628/628324.png'/>
            <h1 style="justify:center;place-self: center;margin: 0px; font-family: 'brandon-grotesque'; text: center;letter-spacing: 0em;text-transform: uppercase;">Plantasynch</h1>
          </div>`;
     ...
     padding-right: 0.5rem">Keep your plants alive</p>
      ...
      if (plants.length) {
        for (let plant of plants) {
          plantHtmlComponent += `<div><img style="margin-top: 18px ;width:300px;height:300px" src='${
            plant.image
          }' /><h4 style="place-self: left;margin: 0px;padding: 5px;">Name: ${
            plant.name
          }</h4><h4 style="margin: 0px;padding: 5px;">Last Watered: ${dayjs(
            plant.last_water
          ).format(
            'MMM D, YYYY'
          )}</h4><h4 style="margin: 0px;padding: 5px;">Category: ${
            plant.category
          }</h4>
