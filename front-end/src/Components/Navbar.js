...
export default function Navbar({notification}){
  ...
  return (
    <div className="fixed bottom-0 z-40 flex flex-row place-content-evenly  tablet:place-content-evenly w-screen h-24 bg-green-200 shadow-lg laptop:laptop-display laptop:place-self-center">
      ...
      <Link to={'/new'} className={`flex flex-col place-content-center place-items-center ${location.pathname.startsWith('/new') ? 'text-black' : 'text-gray-300'} ${effect.add && "animate-pulse"}`} onClick={() => {setEffect({...effect, add: true });
      ...
      <Link to={'/'} className={`flex flex-col place-content-center place-items-center ${location.pathname === '/' || location.pathname === '/sign-in' ? 'text-black' : 'text-gray-300'} ${effect.welcome && "animate-pulse"}`} onClick={() => {setEffect({...effect, welcome: true });
