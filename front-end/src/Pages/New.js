...
export default function New({loggedInUser}) {
  ...
  return (
    <div>
      <div className="flex flex-row p-3 pb-0 tablet:justify-center tablet:px-[15%]">
        ...
        {toggleAdd.scanView ? 'Scan my Plant' : 'Add my Plant'}
            ...
            <div className='flex w-full place-self-center flex-row max-w-[500px] max-h-fit'>
              <div className={`max-w-[250px] w-[50%] absolute ease-in-out transition-transform duration-300 opacity-90 flex text-[#D9F8B9] ${toggleAdd.formView ? 'translate-x-[90%] tablet:translate-x-full pl-2 pr-1 tablet:pl-2 tablet:pr-0' : 'pr-2 tablet:pr-0'}`}>
                <div className={'max-w-[250px] w-full h-[45px] bg-[#224722] rounded-xl'}></div>
              </div>
